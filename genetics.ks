...
function ga_save {
  ...
  set str ti str:substring(0,str:length-1)+").".
  
  log "" to ga_data.ks. delete ga_data.ks.
  log str to ga_data.ks. 
}

// Set up a nre test with starting chromosomes
function ga_init {
  parameter chroms.
  set ga_gen to 1.
  set ga_data to list().
...
// Select parent using roulett-wheel selection
function ga_select {
  ...
  until parents:length = len {
    local item is data[float(random()*len)].  
